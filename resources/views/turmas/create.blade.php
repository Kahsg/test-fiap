            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body pb-0">
                        <form class="mb-10" action="{{ route('turmas.store') }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="form-group col-6 mt-2">
                                    <label for="nome" class="list-label">Nome da turma:</label>
                                    <input type="text" class="form-control @error('nome') is-invalid @enderror" name="nome" value="{{ old('nome') }}" placeholder="Ex.: Administração - Presencial" required>
                                    @error('nome')
                                        <div class="">
                                ...
                                <div class="form-group col-6 mt-2">
                                    ...
                                    required>
                                        ...
                                        @foreach ($tipos as $tipo)
                                            <option value="{{ $tipo->uuid }}" {{ old('tipo') == $tipo->uuid ? 'selected' : '' }}>{{ $tipo->descricao }}</option>
                                        @endforeach
                                    </select>
                                    @error('tipo')
                                        <div class="">
                                ...
                                <div class="form-group col-12 mt-2">
                                    ...
                                    <input type="text" class="form-control @error('descricao') is-invalid @enderror" name="descricao" value="{{ old('descricao') }}" placeholder="Ex.: Bacharelado 4 anos" required>
                                    @error('descricao')
                                        <div class="">
                                            <p class="text-danger">{{ $message }}</p>
                                        </div>
